...
- ```for문```은 특정 숫자를 가지고 숫자의 값을 비교하고, 증감해주면서 반복한다면, ```while문```은 **조건을 확인만 하면서 반복한다. 따라서 조건문 내부에서 변화를 직접 주어야한다.**
...
```
- while문을 사용 할 때에는 조건문이 언젠간 false 해야한다. false 로 전환이 되지 않는다면 반복문이 끝나지 않고 계속 실행된다.
